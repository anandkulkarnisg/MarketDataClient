...
public class c {
	...
	public byte[] fake(byte[] b, boolean littleEndian) {
		this.b = b;
		this.a = littleEndian;
		u();
		return b;
	...
	public Object r() throws UnsupportedEncodingException {
		int i = 0, n, t = b[j++];
		if (t < 0)
			switch (t) {
				case -1:
	...
	static int[] nt = { 0, 1, 0, 0, 1, 2, 4, 8, 4, 8, 1, 0, 8, 4, 4, 8, 8, 4,
		4, 4 };
	...
	public static Object[] NULL = { null, new Boolean(false), null, null,
		new Byte((byte) 0), new Short(Short.MIN_VALUE), new Integer(ni),
		new Long(nj), new Float(nf), new Double(nf), new Character(' '),
		"", new Timestamp(nj), new Month(ni), new Date(nj),
		new java.util.Date(nj), new Timespan(nj), new Minute(ni),
		new Second(ni), new Time(nj) };

	public static Object NULL(char c) {
		return NULL[" b  xhijefcspmdznuvt".indexOf(c)];
	}
